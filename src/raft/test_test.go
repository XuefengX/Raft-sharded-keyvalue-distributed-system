...
func TestReElection2A(t *testing.T) {
	...
	// be elected.
	DPrintf("Should has no leader after disconnect leader2 %d and follower %d", leader2, (leader2+1)%servers)
	...
	DPrintf("Should have a leader")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf("Rejoin leader2, should have one leader")
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestFailAgree2B(t *testing.T) {
	...
	DPrintf("Send with all servers on")
