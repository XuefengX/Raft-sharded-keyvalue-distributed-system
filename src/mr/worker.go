...
func Worker(mapf func(string, string) []KeyValue,
	...
	for {
		...
		commitArgs := CommitArgs{WorkerID: workerID, TaskID: reply.TaskID, Mode: reply.Mode}
...
func MapWork(task WorkReply, mapf func(string, string) []KeyValue) {
	...
	for _, kv := range kva {
		key := kv.Key
		reduce_idx := ihash(key) % task.BucketNum
...
func ReduceWork(task WorkReply, reducef func(string, []string) string) {
	...
	for mapTaskNumber := 0; mapTaskNumber < task.BucketNum; mapTaskNumber++ {
